...
_Benchmark 470_:  
  ...
  5. _`l2_misses : 0.999940`_  
    ...
    <pre>Περιγραφή: Γενικός κανόνας είναι ότι όταν έχουμε μεγάλο αριθμό Misses στις διάφορες μνήμες (i_cache, d_cache, l2_cache) έχουμε
    και μεγάλο CPI, το οποίο συνεπάγεται σε χειρότερο (μεγαλύτερο) χρόνο εκτέλεσης της προσομοίωσης του benchmark.
    Συγκεκριμένα στο παράδειγμα μας το benchmark 458 έχει μεγάλες τιμές Misses που συνεπάγεται σε μεγάλο
    CPI και άρα μεγάλο χρόνο εκτέλεσης της προσομοίωσης. Αντίθετα, το benchmark 456 έχει τον μικρότερο αριθμό Misses
    σε όλες τις μνήμες, που αντιστοιχίζεται στο μικρότερο CPI και άρα στον καλύτερο (μικρότερο) χρόνο προσομοίωσης.
    </pre>
   <br><br>
...
Για όλα τα benchmarks ισχύει:  
  ...
  <pre>Τα διάφορα μέρη του συστήματος χρονίζονται στο 1GHz, ενώ οι πυρήνες του συστήματος και όποιοι επεξεργαστές προστίθενται στην συνέχεια χρονίζονται στα 2GHz. Επαλήθευση αυτών των παρατηρήσεων είναι αυτά τα δύο κομμάτια κώδικα, που βρίσκονται στο .json αρχείο: 
  "name": "cpu_clk_domain", 
            "clock": [
                500
            ],
